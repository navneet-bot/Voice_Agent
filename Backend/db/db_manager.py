    ...
    async def list_agents(self, client_id: Optional[str] = None) -> list[dict]:
        def _sync():
            ...
            try:
                if client_id:
                    rows = conn.execute(
                        "SELECT * FROM agents WHERE client_id=? AND deleted_at IS NULL ORDER BY created_at DESC",
                        (client_id,)
                    ).fetchall()
                else:
                    rows = conn.execute("SELECT * FROM agents WHERE deleted_at IS NULL ORDER BY created_at DESC").fetchall()
                result = []
                for r in rows:
                    d = dict(r)
    ...
    async def soft_delete_agent(
        ...
        *,
        ...
        def _sync():
            conn = _get_connection()
            try:
                existing = conn.execute("SELECT id FROM agents WHERE id=? AND deleted_at IS NULL", (agent_id,)).fetchone()
                if not existing:
                    return None
