...
- "not interested", "no requirement", "don't need property", "not really" -> intent: "deny_interest"
  - Hindi/Devanagari: "मुझे इंट्रेस्ट नहीं है", "रुचि नहीं है", "मुझे नहीं चाहिए", "कोई ज़रूरत नहीं" -> intent: "deny_interest"
...
_DENIAL_TEXTS = {"no", "nope", "nah", "nahi", "nai", "na", "nako", "नहीं", "नही", "ना", "ने ही", "नको", "नाही"}
_BUSY_HINTS = (
...
_NOT_INTERESTED_HINTS = (
    ...
    "मुझे इंट्रेस्ट नहीं है", "मुझे इंट्रेस्ट नही है", "रुचि नहीं है",
    "कोई ज़रूरत नहीं", "नहीं चाहिए", "नको", "नको गरज",
)
...
def _classify_local_intent(user_text: str) -> dict[str, Any] | None:
    ...
    # Handle repeated words from STT like "no no", "yes yes", "nahi nahi"
    words = clean_text.split()
    if len(words) > 1 and len(set(words)) == 1:
        word = words[0]
        if word in _DENIAL_TEXTS:
            entities["confirmation"] = "no"
            return {"intent": "deny", "entities": entities}
        if word in _CONFIRMATION_TEXTS:
...
async def generate_response(
    ...
    if state_manager and state_manager.schema:
        schema = state_manager.schema
        turn.context.setdefault("agent_name", schema.get("agent_name", "Neha"))
        agent_type = (schema.get("agent_metadata") or {}).get("agent_type", "real_estate_sales")
        team_map = {
        ...
        turn.context.setdefault("team_name", team_map.get(agent_type, "Customer advisory team"))
