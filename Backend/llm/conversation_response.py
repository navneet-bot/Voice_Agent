    ...
    *,
    ...
    if language == "hi":
        ...
        if question == "off_topic":
            return f"Main sirf {topic} ke baare mein help kar sakti hoon."
        return f"Aapki earlier {topic} interest ke baare mein call kiya hai."
    ...
    if language == "mr":
        ...
        if question == "off_topic":
            return f"मी फक्त {topic}बद्दल मदत करू शकते."
        return f"तुमच्या earlier {topic} interest बद्दल call केला आहे."
    if question == "identity":
        return f"{agent_name} here from the {team_name}."
    if question == "off_topic":
        return f"I can only help with {topic}-related questions."
    return f"It's about your earlier {topic} interest."
