...
LOCALIZED_TEMPLATE_MAP = {
    ...
    "I'm reaching out regarding some financial solutions that might interest you. Do you have a moment to discuss?": {
        "hi": "Financial solutions ke baare mein call kiya hai jo aapke liye useful ho sakte hain. Ek minute mil jayega?",
        "hinglish": "Financial solutions ke baare mein call kiya hai jo aapke liye useful ho sakte hain. Ek minute milega?",
    },
    "I'm calling about insurance plans that could be relevant for you. Would you have a moment?": {
        "hi": "Insurance plans ke baare mein call kiya hai jo aapke liye relevant ho sakte hain. Ek minute mil jayega?",
        "hinglish": "Insurance plans ke baare mein call kiya hai jo aapke liye relevant ho sakte hain. Ek minute milega?",
    },
    "I'm reaching out regarding educational guidance and course options. Do you have a moment to discuss your interests?": {
        "hi": "Education guidance aur course options ke baare mein baat karni thi. Ek minute mil jayega?",
        "hinglish": "Education guidance aur course options ke baare mein baat karni thi. Ek minute milega?",
    },
...
class LanguageTracker:
    ...
    def observe(self, text: str) -> tuple[str, UserTextAnalysis]:
        ...
        # Make switching symmetrical — no hard stickiness to non-English
        is_returning_to_english = detected == "en" and self.current_language in {"hi", "mr", "hinglish"}

        # Same confidence threshold for both directions
        if is_returning_to_english:
            strong_switch = analysis.confidence >= 0.85
        else:
        ...
        # Symmetric: same number of hits required in both directions
        required_hits = 2
...
def analyze_user_text(text: str, fallback: str = "en") -> UserTextAnalysis:
    ...
    if devanagari_letters > 0:
        ...
        # Pure Devanagari text with 0 markers is likely STT noise — low confidence so it doesn't force a language switch
        confidence = 0.78 if latin_letters == 0 else 0.84
        return UserTextAnalysis(text, cleaned, "hi", confidence, True, "devanagari", latin_letters, devanagari_letters, unsupported_letters)
