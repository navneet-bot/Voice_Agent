...
SHORT_NOISE = {".", ",", "uh", "ah", "hmm", "hm", "um", "oh", "ohh", "this", "that", "jaal", "झाल", "haal", "ढाल"}
NON_SKIPPABLE_NAMES = {
    "Smart Greeting",
    "Confirm and End",
    "Confirm Callback",
...
class StateManager:
    ...
    def _repair_generated_greeting(self, flow: dict[str, Any]) -> None:
        ...
        agent_type = str(
            (self.schema.get("agent_metadata") or {}).get("agent_type") or "real_estate_sales"
        )
        discovery_response_map = {
            "real_estate_sales": "I'm calling about some premium property options. Would you have a moment?",
            "finance": "I'm reaching out regarding some financial solutions that might interest you. Do you have a moment to discuss?",
            "insurance": "I'm calling about insurance plans that could be relevant for you. Would you have a moment?",
            "education": "I'm reaching out regarding educational guidance and course options. Do you have a moment to discuss your interests?",
        }
        discovery_instruction_map = {
            "real_estate_sales": "Qualify lead interest as a real estate sales agent and collect data fields.",
            "finance": "Qualify lead interest as a finance advisory agent and collect data fields.",
            "insurance": "Qualify lead interest as an insurance advisory agent and collect data fields.",
            "education": "Qualify lead interest as an education counselling agent and collect data fields.",
        }
        expected_discovery_response = discovery_response_map.get(
            agent_type, discovery_response_map["real_estate_sales"]
        )
        expected_discovery_instruction = discovery_instruction_map.get(
            agent_type, discovery_instruction_map["real_estate_sales"]
        )
        for node in nodes:
            ...
            elif node.get("id") == "discovery":
                # Repair stale hardcoded "premium property" text when agent_type is different.
                response_text = node.get("response") or ""
                if "premium property" in response_text and agent_type != "real_estate_sales":
                    node["response"] = expected_discovery_response
                instruction = node.get("instruction")
                if isinstance(instruction, dict):
                    text = instruction.get("text") or ""
                    if "real estate sales agent" in text and agent_type != "real_estate_sales":
                        instruction["text"] = expected_discovery_instruction
    ...
    def template_new_agent(
        ...
        discovery_response = {
            ...
            "insurance": "I'm calling about insurance plans that could be relevant for you. Would you have a moment?",
            "education": "I'm reaching out regarding educational guidance and course options. Do you have a moment to discuss your interests?",
        }.get(agent_type, "I'm calling about some options that might interest you. Would you have a moment?")
            ...
            "conversationFlow": {
                ...
                "nodes": [
                    ...
                    },
                    {
                        ...
                        "type": "conversation",
                        "instruction": {"type": "prompt", "text": f"Qualify lead interest as a {agent_type.replace('_', ' ')} agent and collect data fields."},                        
    ...
    def execute_transition(self, user_text: str, intent_data: Optional[dict[str, Any]]) -> TurnResult:
        ...
        # ── Resolve next node ────────────────────────────────────────────
        if intent in {"confirm", "confirm_identity"} or intent in ALL_DENY_INTENTS:
        ...
        if next_node.get("type") == "fallback":
            ...
            if self._fallback_counts[node_id] >= MAX_FALLBACK_ATTEMPTS:
                _log("FALLBACK EXCEEDED", f"{node_id} hit limit, forcing end")
                end_node = self.nodes.get(POLITE_END_NODE_ID)
                if end_node:
                    next_node = end_node
    ...
    def _handle_confirmation(self, current_node: dict[str, Any], intent: str) -> tuple[dict[str, Any], bool]:
        ...
        if not edge:
            # No matching confirm/deny edge found.
            # For slot-collection nodes, stay put — don't advance via fallback edge.
            # This prevents "Okay" on a discovery node from routing to an end node.
            collects = self._collect_slots(current_node)
            if collects:
                return current_node, False
            # Fallback to first forward edge for non-collection nodes
            # (handles custom agent schemas with generic conditions like "user responds")
            forward_id = self._first_destination(current_node)
            if forward_id:
                forward_node = self.nodes.get(forward_id)
                if forward_node:
                    _log("STATE", f"{current_node['id']} -> {forward_id} (confirm fallback edge)")
                    return forward_node, False
            return current_node, False
        destination_id = edge.get("destination_node_id")
        destination = self.nodes.get(destination_id)
        if not destination:
    ...
    def _route_deny_subtype(
        ...
        if intent == "deny":
            ...
            # Check start node first (handles any schema, including FlowSpec v2)
            if node_id == self.start_node_id:
                intent = "deny_identity"
            elif node_id in DENY_IDENTITY_NODES:
                intent = "deny_identity"
        ...
        if intent == "deny_identity":
            ...
            if target:
                ...
                return target
            # Fallback to any end node if schema doesn't have specific one
            target = self._find_any_end_node()
            if target:
                _log("DENY ROUTE", f"deny_identity -> {target['id']} (fallback end)")
                return target
        ...
        if intent == "deny_interest":
            ...
            target = self._find_any_end_node()
            if target:
                _log("DENY ROUTE", f"deny_interest -> {target['id']} (fallback end)")
                return target
    ...
    def _find_any_end_node(self) -> Optional[dict[str, Any]]:
        """Find any node with type==end as fallback."""
