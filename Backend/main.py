...
AGENT_TYPE_DISCOVERY_RESPONSES = {
...
AGENT_TYPE_DISCOVERY_INSTRUCTIONS = {
...
def _write_agent_runtime_schema(
    ...
    agent_type = agent_data["agent_type"]
    discovery_response = AGENT_TYPE_DISCOVERY_RESPONSES.get(
        agent_type,
        "I'm calling about some options that might interest you. Would you have a moment?",
    )
    discovery_instruction_text = AGENT_TYPE_DISCOVERY_INSTRUCTIONS.get(
        agent_type,
        f"Qualify lead interest as a {agent_type.replace('_', ' ')} agent and collect data fields.",
    )

    for node in nodes:
        if node.get("id") == "discovery":
            node["collects"] = agent_data["data_fields"]
            node["response"] = discovery_response
            node["instruction"] = {
                "type": "prompt",
                "text": discovery_instruction_text,
            }
...
def _build_demo_call_qa_readiness() -> dict:
    ...
    try:
        ...
        purpose_ok = (
            ...
            and "enquiry" in purpose_response.lower()
...
async def delete_agent(agent_id: str, request: Request, data: CampaignLifecycleRequest | None = None):
    agent = await db.get_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent not found")
    result = await db.soft_delete_agent(
        agent_id,
        reason=data.reason if data else None,
        actor_email=_actor_email(request, data.actorEmail if data else None),
    )
    return {"status": "soft_deleted", "agent": result}
